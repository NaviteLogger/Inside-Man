...
#
#   scripts/bootstrap.sh              every tool
#   scripts/bootstrap.sh helm kind    just those
#
# Re-running is a no-op once each tool reports its pinned version. Asking for a
# subset keeps a CI job that only needs helm from downloading a Go toolchain,
# which is both slow and one more upstream that can be having a bad day.
...
ALL_TOOLS=(kubectl kind helm go gitleaks)
if [[ $# -gt 0 ]]; then
  WANTED=("$@")
  for t in "${WANTED[@]}"; do
    case " ${ALL_TOOLS[*]} " in
      *" ${t} "*) ;;
      *) echo "unknown tool: ${t} (known: ${ALL_TOOLS[*]})" >&2; exit 1 ;;
    esac
  done
else
  WANTED=("${ALL_TOOLS[@]}")
...
# --- kubectl -----------------------------------------------------------------
if want kubectl && [[ "$("${BIN_DIR}/kubectl" version --client -o json 2>/dev/null | grep -o "\"gitVersion\": *\"${KUBECTL_VERSION}\"" || true)" ]]; then
  skip kubectl
elif want kubectl; then
...
# --- kind --------------------------------------------------------------------
if want kind && [[ "$("${BIN_DIR}/kind" version 2>/dev/null | grep -o "${KIND_VERSION}" || true)" ]]; then
  skip kind
elif want kind; then
...
# --- helm --------------------------------------------------------------------
if want helm && [[ "$("${BIN_DIR}/helm" version --short 2>/dev/null | grep -o "${HELM_VERSION}" || true)" ]]; then
  skip helm
elif want helm; then
...
# system Go install.
if want go && [[ "$("${BIN_DIR}/go" version 2>/dev/null | grep -o "go${GO_VERSION}" || true)" ]]; then
  skip go
elif want go; then
...
# binary on PATH.
if want gitleaks && [[ "$("${BIN_DIR}/gitleaks" version 2>/dev/null | grep -o "${GITLEAKS_VERSION}" || true)" ]]; then
  skip gitleaks
elif want gitleaks; then
...
log "ready in ${BIN_DIR}: ${WANTED[*]}"
want kubectl  && "${BIN_DIR}/kubectl" version --client 2>/dev/null | head -1
want kind     && "${BIN_DIR}/kind" version
want helm     && "${BIN_DIR}/helm" version --short
want go       && "${BIN_DIR}/go" version
want gitleaks && "${BIN_DIR}/gitleaks" version
true
